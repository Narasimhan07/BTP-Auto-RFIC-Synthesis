...
# the RC tank contains 5 capacitances in parallel
def set_mimcap_um_rf_w_l(CL):
    CL_per_cap = CL/float(5)
...
def extract_gain(ocean_script, netlist_type):
    ...
    set_path = "cd /home/ee20b087/cadence_project/BTP_EE20B087"
    # go to the directory that has the ocean script
    os.system(set_path)
    ...
    if netlist_type=="single_point":
        ...
        CSV_file_path = "freq_analysis_gain.csv"
...
def extract_integrated_NF(ocean_script, return_freq_data):
    ...
    set_path = "cd ~/cadence_project/BTP_EE20B087"
    # go to the directory that has the ocean script
    os.system(set_path)
    # run the ocean script to write data onto results.txt file
    ...
    if return_freq_data==True:
        CSV_file_path = "freq_analysis_NF.csv"
...
def extract_iip3(ocean_scripts):
    ...
    os.system(ocean_command)
    CSV_file_path = "slope_ip3_curve.csv"
...
def write_opt_results(loss_iter, post_iteration_circuit_parameters_iter, alpha, iter_number):
    if iter_number == 1:
        os.system("rm opt_results.txt")
