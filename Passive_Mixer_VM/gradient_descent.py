...
class Circuit:
    ...
    def __init__(self, circuit_initialization_parameters, initial_simulation_parameters):  
    ...
    def run_circuit(self, output_conditions):
        ...
        for flo in flo_array:
            # 1) Adding S11 results at f = flo+bandwidth to the simulated output parameters dict
            # Edit the VM_S11_single_point.scs netlist with the circuit parameters
            cf.S11_netlist_edit(
            ...
            # Edit the VM_gain_single_point.scs netlist with the circuit parameters
            ...
            # Edit the VM_NF_single_point.scs netlist with the circuit parameters
            ...
            # Edit the VM_iip3.scs netlist with the circuit parameters
    ...
    def run_circuit_multiple(self, output_conditions, initial_circuit_parameters_dict):
        ...
        for i in initial_circuit_parameters_dict:
            ...
            for flo in flo_array:
                # 1) Edit the VM_S11_single_point.scs netlist with the circuit parameters
                cf.S11_netlist_edit(
                ...
                # read the output from sp_single_pt.out
                # S11 computed at flo is stored in the dict in simulated output parameters using the key = flo
                simulated_output_parameters_dict[i]['S11_db'][flo] = cf.extract_S11(self.simulation_parameters['S11']['.out_file_path'], "single_point")

                # 2) Edit the VM_gain_single_point.scs netlist with the circuit parameters
                cf.gain_netlist_edit(
                    flo, output_conditions['RF_Bandwidth'], initial_circuit_parameters_dict[i], self.simulation_parameters, "single_point"
                )
                # simulate the spectre netlist
                ...
                # 3) Edit the VM_NF_single_point.scs netlist with the circuit parameters
                cf.integrated_NF_netlist_edit(
                ...
                # 4) Edit the VM_iip3.scs netlist with the circuit parameters
                cf.iip3_netlist_edit(
...
def calc_loss_slope(cir,output_conditions,loss_dict,optimization_parameters):
    ...
    i=0
    for param_name in optimization_parameters['optimizing_variables']:
        print("----------------------------- Calculating slope wrt ", param_name," ---------------------------------\n")
